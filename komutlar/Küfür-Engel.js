...
:x: !kufur-engel aç/kapat Yazmalısın.
`))
let küfür = await db.fetch(`kufurengel_${message.guild.id}`)
if(args[0] === 'aç') {
    if(küfür) {
db.set(`kufurengel_${message.guild.id}`, `acik`)
const aç = new Discord.MessageEmbed()
.setColor('0x36393E')
.setDescription(`✅ Küfür Engel Sistemi Zaten Açık!`)
...
exports.conf = {
  ...
  exports.help = {
    name: 'küfür-engel',
    description: 'Küfür Engel Sistemini Açıp Kapatırsınız.',
    usage: 'küfür-engel'
  };